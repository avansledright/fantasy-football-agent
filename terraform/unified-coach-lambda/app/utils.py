...
def calculate_adjusted_score(
    ...
    """
    # Convert all inputs to float (in case they're Decimal from DynamoDB)
    weekly_proj = safe_float(weekly_proj)
    season_proj_per_game = safe_float(season_proj_per_game)
    recent_avg = safe_float(recent_avg)
    vs_opp_avg = safe_float(vs_opp_avg) if vs_opp_avg is not None else None
