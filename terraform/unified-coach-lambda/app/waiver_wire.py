...
from app.projections import safe_float
...
def _calculate_upside_score(projected: float, ownership: float, season_proj: float, historical: float) -> float:
    ...
    # Convert all parameters to float (in case they're Decimal from DynamoDB)
    projected = safe_float(projected)
    season_proj = safe_float(season_proj)
    historical = safe_float(historical)
